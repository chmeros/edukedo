...
- **Admin-/Redaktionsbereich, erste Version (F-11):** `src/AdminPanel.tsx` — Kursliste mit Veröffentlichen/Zurückziehen-Button je Kurs, nur sichtbar für `role === "admin"` (`App.tsx`, kein eigener Account-Typ, keine eigene Route). Löst den bisherigen Weg ab, `kurs.is_published` ausschließlich per SQL zu setzen.
