...
- **tRPC-Router-Grundstruktur:** `src/trpc/router.ts` (`health`, `auth`, `courses`, `content`, `progress`, `quiz`, `consent`, `parent`; weitere Module folgen modulweise).
- **Noch offen:** automatische Erinnerungsmails und kontoloser Vorschau-Modus (F-08), granulare Kind-Berechtigungen im Eltern-Dashboard (F-90, setzt Gamification F-66 voraus), Admin-Content-Router, Sozial-Modul (Phase 4).
