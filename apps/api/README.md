...
- **Quiz-Modus (F-21, bisher nur Multiple Choice):** Router `quiz` (`quizItems` liefert Fragen ohne die richtige Antwort, `submitAnswer` prüft serverseitig und gibt Feedback + Erklärung zurück). Zuordnung/Lückentext folgen als Erweiterung.
- **tRPC-Router-Grundstruktur:** `src/trpc/router.ts` (`health`, `auth`, `courses`, `content`, `progress`, `quiz`; weitere Module folgen modulweise).
- **Noch offen:** Consent-/Eltern-Flow (Iteration 2), Zuordnung/Lückentext im Quiz-Modus, Admin-Content-Router, Sozial-Modul (Phase 4).
