...
- **Admin-/Redaktionsbereich, erste Version (F-11):** Router `admin` — `admin.courses` (alle Kurse unabhängig vom Veröffentlichungsstatus) und `admin.setPublished` (Kurs veröffentlichen/zurückziehen), beide über `roleProcedure("admin")` gesichert (`user.role = "admin"`, nur per direktem SQL-Update setzbar). Löst den bisherigen Weg ab, `kurs.is_published` ausschließlich per SQL zu setzen. Fragen-/Karteikarten-Pflege und ein Bulk-Import-Trigger (F-17) bleiben spätere Ausbauschritte. Siehe Architekturplanung Abschnitt 13.
