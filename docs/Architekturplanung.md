...
- **Offline-Quiz-Antworten werden inklusive Lösung heruntergeladen (Nutzer-Entscheidung), statt das Feedback erst beim Sync anzuzeigen.** Das weicht von der bestehenden Regel "die richtige Option wird den Lernenden serverseitig nie mitgeschickt" (siehe `quiz.ts`) für den Sonderfall explizit heruntergeladener Offline-Inhalte ab: `checkMcAnswer`/`checkMatching`/`checkBlanks`/`checkKurzantwort` (`apps/api/src/quiz-logic.ts`) brauchen die Lösung, um offline sofort prüfen zu können, ohne die der Kern-Lerneffekt (sofortiges Feedback) entfiele. Der Kompromiss — wer die lokalen Daten inspiziert, sieht die Lösungen der heruntergeladenen Fragen im Voraus — wurde als vertretbar eingestuft, da edukedo eine kostenfreie, nicht-kompetitive Lernplattform ohne Prüfungscharakter ist (kein Anti-Cheat-Interesse wie bei einer echten Prüfung). `quiz-logic.ts` selbst ist dafür noch nicht nach `@edukedo/shared` verschoben — das passiert erst mit Baustein 4 (Offline-Antwortpfad), `offlineDb.ts` ist aber bereits so typisiert (`OfflineAnswerOption` deckungsgleich mit `RawAnswerOption`), dass das ohne Schema-Änderung nachgezogen werden kann.
