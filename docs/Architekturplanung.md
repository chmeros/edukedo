...
### Entschieden am 16.09.2026 (F-42 Offline-Modus, Architekturansatz + erste fünf Bausteine)

Mit dem Nutzer abgestimmter Gesamtansatz (sechs Bausteine, in dieser Reihenfolge umzusetzen — Baustein 1–5 bereits erledigt, siehe Entwicklungsplan Iteration 5):
...
- **Ein einzelner fehlerhafter Eintrag bricht nicht den gesamten Sync-Batch ab:** Wird ein `QuizItemNotFoundError` (Frage/Option existiert nicht mehr) oder ein Zod-Parse-Fehler (Payload-Struktur hat sich seit dem Download geändert) geworfen, bleibt nur dieser eine Eintrag unsynchronisiert (fehlt in `syncedIds`, der Client behält ihn in der lokalen Warteschlange) — alle anderen Einträge werden trotzdem übernommen. Ein serverseitig zwischenzeitlich deaktiviertes Content-Item darf nicht den kompletten restlichen Lernfortschritt eines Sync-Versuchs blockieren.
