...
- [ ] Quiz-Modus (F-21) mit Sofort-Feedback — Multiple-Choice-Teil bereits umgesetzt (Backend `apps/api/src/trpc/routers/quiz.ts`, Frontend `apps/web/src/Quiz.tsx`, Ende-zu-Ende gegen echtes Postgres verifiziert; richtige Antwort wird serverseitig erst bei der Auswertung offengelegt); Zuordnung und Lückentext (ebenfalls Teil von F-21) folgen als nächste Schritte, bevor dieser Punkt vollständig abgehakt wird
